    ...
    {{ category.name }}
            ...
            <div class="form-field form-field--full">
              ...
              <textarea
                ...
                rows="6"
                placeholder="請輸入商品特色、材質、規格與使用說明"
                :disabled="isSubmitting"
              />
            </div>
          </div>
        ...
        <section class="form-card">
          <div class="form-card__header">
            <div>
              ...
              <p>
                目前先使用圖片網址，下一階段再串接 Supabase Storage 上傳。
              </p>
            </div>
          </div>
...
const mallStore =
  ...
  useMallCategoryStore()
onMounted(async () => {
  await mallCategoryStore.fetchEnabledCategories()
})
const isSubmitting =
  ref(false)
...
const form =
  reactive<CreateMallProductInput>({
    productCode: '',
    name: '',
    subtitle: '',
    description: '',
...
function validateForm():
  ...
  if (
    ...
    return '分類 ID 必須是正確的 UUID，或保持空白。'
  }
  ...
  if (
    ...
    return '品牌 ID 必須是正確的 UUID，或保持空白。'
  }
